...
class StoreEvaluasiRequest extends FormRequest
{
    ...
    {
        // Only Kaprodi and Admin can create evaluasi
        return $this->user()->hasRole(['kaprodi', 'admin']);
    }
    ...
    public function rules(): array
    {
        $rules = [
            ...
            'semester' => ['required', 'in:' . Evaluasi::SEMESTER_GANJIL . ',' . Evaluasi::SEMESTER_GENAP],
            'tahun_evaluasi' => ['required', 'integer', 'min:2020', 'max:' . (date('Y') + 5)],
    ...
    protected function prepareForValidation(): void
    {
        ...
        // Set prodi_id to current user's prodi if not admin
        if (!$this->user()->isAdmin() && !$this->has('prodi_id')) {
            $this->merge([

                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($targets as $target)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 text-sm text-gray-900">{{ $target->indikator?->nama_indikator ?? '-' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ number_format($target->target_value, 2) }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $target->satuan }}</td>
                                        <td class="px-6 py-4 text-sm text-gray-500">{{ Str::limit($target->keterangan, 50) }}</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="5" class="px-6 py-4 text-center text-sm text-gray-500">Tidak ada data target.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
